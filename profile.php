...
?>
<!-- <form action="profile.php?username=" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
				...
				<?php
					if ($userid == $usrid) {
						...
						<textarea name='postbody' rows='8' cols='30' class='btn btn-default' ></textarea>
						<br />Upload an image:
						<input type='file' name='postimg'>
						<input class='btn btn-default' type='submit' name='post' value='Post' style='width:240px;background-image:url(&quot;none&quot;);background-color:#da052b;color:#fff;padding:16px 32px;margin:0px 0px 6px;border:none;box-shadow:none;text-shadow:none;opacity:0.9;text-transform:uppercase;font-weight:bold;font-size:13px;letter-spacing:0.4px;line-height:1;outline:none;'>
						</form>";
					}					
				?>	
...
<div class="msg_box" style="left:110px">
	...
	<div class="msg_wrap">
		...
		<?php
				$a=1;
				$messages = DB::query('SELECT messages.*, users.username FROM messages, users WHERE receiver=:receiver AND users.id = messages.sender', array(':receiver'=>$usrid));
